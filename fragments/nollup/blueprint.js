    ...
    configs: ({ $require }) => {
        return {
            nollup: {
                ...
                publicPath: $require('relative')($require('distDir') + ', ' + $require('buildDir')) + ', // build',
                historyApiFallback: $require('relative')($require('assetsDir') + ', ' + $require('template')) + ', // __app.html',
                port: $require('port'),
            },
            rollup: {
                plugins: [
                    `!production && isNollup && ${$require('Hmr')}({ inMemory: true, public: ${$require('assetsDir')}, }), //, refresh only updated code`,
